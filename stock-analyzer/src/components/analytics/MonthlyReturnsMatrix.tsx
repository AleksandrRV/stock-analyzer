...
import { PortfolioCalculationService } from '../../services/engine/PortfolioCalculationService';
...
export const MonthlyReturnsMatrix: React.FC<Props> = ({ portfolio }) => {
  const { settings, matrixCache, setMatrixCache } = usePortfolioStore();
  
  const currentHour = new Date().getHours();
  const cachedWrapper = matrixCache[portfolio.id];
  ...
  useEffect(() => {
    ...
    PortfolioCalculationService.calculateMonthlyReturnsMatrix(portfolio, settings)
      .then(rows => {
        if (isMounted) {
          setMatrixCache(portfolio.id, rows, currentHour);
          setLoading(false);
        }
      })
      .catch(err => {
  ...
  }, [portfolio, settings, isCacheValid, currentHour, setMatrixCache]);

  const matrixRows = isCacheValid ? cachedWrapper.result : [];
  ...
  if (loading) {
    return (
      <div className="p-8 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/80 rounded-2xl flex flex-col items-center justify-center gap-2 text-xs font-mono text-slate-400">
        ...
        <span>Загрузка котировок и расчет показателей по каждому месяцу...</span>
      </div>
    );
  }
  ...
  const getAlphaStyle = (alpha: number | null) => {
    ...
    if (alpha > 0.9) return 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/30';
    if (alpha >= 0.01) return 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/30';
              ...
              <div className="grid grid-cols-[repeat(auto-fill,minmax(105px,1fr))] gap-2.5">
                {row.months.map((cell, idx) => {
                  if (cell.portfolioReturn === null || cell.alpha === null) {
                    return (
                      <div key={idx} className="p-2.5 bg-white/40 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800/60 rounded-xl text-center text-slate-300 dark:text-slate-700 text-xs font-mono flex flex-col justify-center items-center min-h-[86px]">
                  ...
                  return (
                    <div key={idx} className={`p-2.5 rounded-xl border flex flex-col justify-between space-y-1.5 transition-all shadow-xs ${getAlphaStyle(cell.alpha)}`}
                      title={`Период: ${cell.startDateDisplay} — ${cell.finishDateDisplay}\nПортфель: ${cell.portfolioReturn >= 0 ? '+' : ''}${cell.portfolioReturn}%\nMCFTR: ${cell.mcftrReturn! >= 0 ? '+' : ''}${cell.mcftrReturn}%\nАльфа: ${cell.alpha >= 0 ? '+' : ''}${cell.alpha}%${cell.isPartial ? '\n(Неполный месяц)' : ''}`}
                    >
                      <div className="flex items-center justify-between text-xs font-semibold font-mono opacity-80 border-b border-current/15 pb-1">
                        ...
                        {cell.isPartial && <span className="text-amber-500 font-bold" title="Неполный месяц">*</span>}
                      </div>
